        ...
        {tab === 'mensajes' && (
          <div className="space-y-3">
            {mensajesEnviados.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm text-center py-12 px-6">
                <div className="text-4xl mb-3">💬</div>
                <p className="font-semibold text-ink/60 text-sm">No enviaste mensajes todavía</p>
                <p className="text-ink/35 text-xs mt-1">Cuando contactes un AT, el mensaje aparecerá acá.</p>
              </div>
            ) : (
              mensajesEnviados.map(m => (
                <div key={m.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-semibold text-sm text-ink">Para: {m.receiver_id ?? 'AT'}</span>
                    <span className="text-xs text-ink/30">
                      {new Date(m.created_at).toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}
                    </span>
                  </div>
                  <p className="text-ink/60 text-sm leading-relaxed">{m.content}</p>
                </div>
              ))
            )}
          </div>
